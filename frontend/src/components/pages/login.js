...
import { Eye, EyeOff, Mail, Lock, User, ArrowRight, ArrowLeft, Globe, Hotel, Compass } from 'lucide-react';
...
function Login() {
    ...
    const toggleMode = () => {
        ...
        navigate(nextIsRegister ? '/register' : '/login', { replace: true });
    };

    const handleGoogleLogin = () => {
        try {
            // Remove setLoading(true) so modal doesn't show before Google redirect
            loginWithGoogle();
        } catch (err) {
